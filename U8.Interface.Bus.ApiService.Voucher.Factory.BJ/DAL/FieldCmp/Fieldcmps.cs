...
namespace U8.Interface.Bus.ApiService.DAL.CQ
{
    ...
    {  
        ...
        public List<Model.Fieldcmps> GetListFieldcmps(Model.Synergismlogdt dt, Model.Synergismlogdt pdt,int tasktype)
        {
            ...
            Model.Fieldcmp fdmodel = fddal.GetModel(string.Format("cfrombillcardnum='{0}' and ctobillcardnum='{1}' and tasktype = {2} ", pdt.Cvouchertype, dt.Cvouchertype, tasktype));
            ApiService.BLL.Common.ErrorMsg(fdmodel, "未能取字段对照数据");
            // 模扳数据
            List<Model.Fieldcmps> listfd = GetModelList(string.Format("id='{0}'   order by seq desc,ctype ,cremark desc,autoid asc", fdmodel.Id));
            ApiService.BLL.Common.ErrorMsg(listfd, "未能取字段对照数据");
            return listfd;
        }
